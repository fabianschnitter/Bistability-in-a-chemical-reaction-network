%experimental data implemented: 25 캜 conc. and turbidity, spike/no spike,
%Fluxes 1, 3, 6, 9
...
Temperature = 25;
Fuel = 50;                  %amount EDC added in mM at the beginning (0/50)
Acid = 10;                 %amount precursor added in mM at the beginning

fluxEDC2 = 9;              %EDC added in mM every minute
fluxAcid2 = 6;             %Acid added in mM every minute

t = 30 * min;              %set time of experiment in minutes and transfer into seconds

v = 0.75;                  %define reaction volume in mL

startpulse = 1*60;         %start fluxes/pumps after 1 minute of initual EDC addition
timeup = 30*min;           %turn off pumps after 20 minutes

%%%experimental conditions set
flowrate_out = 2*0.12*1000/60;         %define flowrate out of reaction vessel in 킠/s
flowrate_EDC = 0.12*1000/60;           %define flowrate in EDC in 킠/s
flowrate_Acid = 0.12*1000/60;          %define flowrate in Precursor in 킠/s
...
[TurbidityTime, Turbidity, HPLCtime, HPLCedc, HPLCanhydride, HPLCprecursor, Sol, CAC, k0, k1, k2, k3, k4] = data_loader_C6(Fuel,Acid,Temperature,fluxEDC2);
